"""Tests for Focus agent memory integration (Week 2 Day 4)."""
...
async def test_focus_includes_semantic_memory_in_llm_payload() -> None:
    ...
    mock_store.search_similar = AsyncMock(return_value=[semantic_hit])
    ...
    assert any(entry.memory_layer == "semantic" for entry in memory_audit_trail.entries)


@pytest.mark.asyncio
async def test_focus_continues_when_semantic_retrieval_fails() -> None:
    mock_store = AsyncMock(spec=SemanticMemoryStore)
    mock_store.search_similar = AsyncMock(side_effect=RuntimeError("db unavailable"))
